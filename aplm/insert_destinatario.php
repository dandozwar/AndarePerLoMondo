...
<html lang="it">
	<head>
		...
		<script src="./js/insert_destinatario.js"></script>
		<script src="./js/background.js"></script>
		<title>Andare per lo Mondo</title>
		<meta name="description" content="Andare per lo mondo: aggiungi destinatario"/>
		...
		<header>
			...
			<ul id = "breadcrumb" class="breadcrumb">
				<?php
					...
					echo '<li>Aggiungi destinatario</li>';
				?>
			</ul>
		</header>
		<div>
			<?php
				...
				echo '<div>
					<h1>Aggiungi destinatario o fruitore</h1>
					<p>La persona a cui lo scopo del viaggio si rivolgeva.<br/>Scegli un destinatario o un fruitore dall&#8217;elenco o cercalo nel database.</p>
					<h3>Attenzione!</h3>
					...
					<div id="dest_scopo">
							<div class="dropdown">
								...
								<div id="ddc_dest_scopo" class="dropdown_content">';
				$q = $conn->query('SELECT id, nome, cognome FROM Persona ORDER BY COALESCE(data_nascita, data_morte)');
				$totpers = $q->num_rows;
				for ($p = 0; $p < $totpers; $p++) {
					$pers = $q->fetch_row();
					echo '<div><input type="radio" id="dest'.$pers[0].'" name="dest_scopo" value="'.$pers[0].'"> <label for="dest'.$pers[0].'">'.$pers[1].' '.$pers[2].'</label></div>';
				};
				echo '</div>
						</div>
						<div class="blank_space"></div>
							<h3>Se non hai trovato quello che cerchi</h3>
							<label for="dest_scopo_search">Cerca nel database:</label>
							</br>
							<input type="text" list="dest_scopo_list" id="dest_scopo_search" name="dest_scopo_search">
							<datalist id="dest_scopo_list">';
				$q = $conn->query('SELECT id, nome, cognome FROM Persona ORDER BY COALESCE(data_nascita, data_morte)');
				$totpers = $q->num_rows;
				for ($p = 0; $p < $totpers; $p++) {
					$pers = $q->fetch_row();
				...
				echo '</datalist>
							...
							<input type="text" id="dest_scopo_selected" hidden="hidden">
						<br/><br/><input type="button" value="Invia" id="invia"  onclick="invia_destinatario('.$scopo.')"/>
						</div>';
			?>
		</div>
		</div>
		<footer>
			<div class="credit">
				<p>
					...
					Progetto del Laboratorio Cultura Digitale - Università di Pisa<br/>
					Tesi Triennale in Informatica Umanistica<br/>
