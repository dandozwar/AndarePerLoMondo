...
function modifica_campo(occupazione, campo) {
	window.location.assign("edit_lavora_come.php?id=" + occupazione + "&campo=" + campo);
};

function invia_campo(idLaco, campo, versione = null) {
	var dati = new FormData();
	var flagErrore = false;
	dati.set('id', idLaco);
	switch (campo) {
		case 'oc':
			dati.set('campo', 'occupazione');
			if (document.querySelector('input[name="attivita"]:checked')) {
				var check_radio = document.querySelector('input[name="attivita"]:checked').value;
				dati.set('valore', check_radio);
			} else if (document.getElementById('attivita_selected').value != "") {
				var check_select = document.getElementById('attivita_selected').value;
	...
	if (flagErrore == false) {
		$.ajax({
			type: "POST",
			url: "php/modifica_campo_lavora_come.php",
			data: dati,
			success: function (resJ) {
				alert(resJ);
				if (resJ == 'Modifica effettuata.') {
					window.location.assign("edit_lavora_come.php?id=" + idLaco);
				};
			},
			processData: false,
			contentType: false,
...
function aggiornaDB(tipo) {
	if (tipo == 0) {
		...
		$.ajax({
			type: "POST",
			url: "php/trova_attivita.php",
			success: function (resJ) {
				var res = JSON.parse(resJ);
				for (occup of res) {
						DBattivita.push(occup);
				};
			},
			processData: false,
			contentType: false,
...
function return_back(nodo) {
	...
	nodoDiv.innerHTML = nodoDiv.innerHTML +
						...
						<div id='ddc_attivita' class='dropdown_content'></div> \
						</div> \
						<div class='blank_space'></div> \
						<h3>Se non hai trovato quello che cerchi</h3> \
						<label for='tipo_attivita'>Cerca nel database:</label> \
						</br> \
	...
	var radio_tire = "";
	var optio = "";
	var rlist_tire = document.getElementById('ddc_attivita');
	var dlist_tire = document.getElementById('attivita_list');
	rlist_occu.innerHTML = "";
	dlist_occu.innerHTML = "";
	for (var a = 0; a < DBattivita.length; a++) {
		radio_occu = "<div><input type='radio' id='at" + DBattivita[a][0] + "' name='attivita' value='" + DBattivita[a][0] + "'> <label for='at" + DBattivita[a][0] + "'>" + DBattivita[a][1] + "</label></div>";
		rlist_occu.insertAdjacentHTML('beforeend', radio_occu);
		optio = "<option data-id='" + DBattivita[a][0] + "' value='" + DBattivita[a][1] + "'>";
...
function apri_insert(nodo) {
	...
	nodoDiv.innerHTML = nodoDiv.innerHTML +
						...
						<form id='insert_attivita'> \
							<label for='tipo'>Attività:</label> \
							</br> \
							<input type='text' id='attivita' name='attivita' maxlength=64'> \
						</form> \
						</br> \
						<input type='button' id='insert_a' value='Inserisci' onclick='insert_tipo(this)'>";
};

function occu_search() {
	var searchId = "attivita_search";
	var listId = "attivita_list";
	var selectId = "attivita_selected";
...
function insert_tipo(nodo) {
	...
	nodoInvia.removeAttribute("hidden");
	var res;
	var form = document.getElementById("insert_attivita");
	...
	if (errori.length == 0) {
		$.ajax({
			type: "POST",
			url: "php/insert_attivita.php",
			data: dati,
			success: function (resJ) {
				if (resJ != 'Attività già inserita.') {
					return_back(nodo);
					res = JSON.parse(resJ);
					var selectId = "attivita_selected";
					var searchId = "attivita_search";
